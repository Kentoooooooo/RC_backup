...
struct Reservoir {
	int trainingNum, testNum, mask_num, node_num, allNum, mask_interval, node_interval, input_interval, maskIndex, nodeIndex, inputIndex;
	double gamma, *mask, **nodeState, *target, *weight, *inputNARMA10, *inputSantaFe, *output, input_signal;
};
...
int main(int argc, char *argv[]) {
	...
	if(out == 1) {
		...
		if(para_flag == 1) {
			...
			fprintf(outputfp[0], "Time [ms]\tx(t)\tMask\tIdeal\tInput\tNode\n");
		} else if(para_flag == 2) {
		} else {
