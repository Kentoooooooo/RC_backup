...
struct Reservoir {
	...
	double gamma, *mask, **nodeState, *target, *weight, *inputNARMA10, *inputSantaFe, *output, input_signal, nyu;
};
...
//	ReadSantaFe(rc);
	...
	init_gen_rand(2121);
	for(i = 0; i < 5000; i++) rc->inputSantaFe[i] = 2 * genrand_open_open() - 1;

	for(i = 0; i < 5000; i++) rc->target[i] = sin( rc->nyu * rc->inputSantaFe[100 + (i - rc->delay_step)] );
