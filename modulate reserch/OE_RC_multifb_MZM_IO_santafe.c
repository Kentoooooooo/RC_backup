...
struct LaserEOfeedback {
	struct parameter p;
	double x, y, *xdelay, buffer, feedback;
	int descretized_num[10], fbNum, fbIndex[10], long_fb;
};

struct Reservoir {
	int trainingNum, testNum, mask_num, node_num, allNum, mask_interval, node_interval, input_interval, maskIndex, nodeIndex, inputIndex, nyu;
	double gamma, *mask, **nodeState, *target, *weight, *inputNARMA10, *inputSantaFe, *output, input_signal, delay_step;
};
...
int main(int argc, char *argv[]) {
	...
	clock_t start, end;
	FILE *inputfp, *outputfp[4], *outputfp_io;
	...
	if(out == 1) {
		...
		if(para_flag == 1) {
			...
			outputfp_io = fopen("MZM in out.txt", "w");
			fprintf(outputfp_io, "Time [\\fm\\ns]\tx(t)\tfeedback\tx(t-\\ft\\n)\tMZM-out\n");
		} else if(para_flag == 2) {
		} else {
		...
		if(para_flag == 3) sprintf(buffer_ch, "Bias point for MZM");
		...
		fprintf(outputfp[0], "%s\tNMSE for training\tNMSE for test\tNormalized entropy\n", buffer_ch);
	...
	for(s = amp_min_int; s <= amp_max_int; s++) {
		...
		for(i = 1; i <= calc_num; i++) {
			...
			rc.input_signal = rc.gamma * rc.mask[rc.maskIndex] * rc.inputSantaFe[rc.inputIndex];
			CalcLaserEOfeedback(&laser, rc.input_signal);

			//calc_modulate from here
			if(out == 1) {
				if(para_flag == 1) {
					if(i % divi == 0) {
						if(i % rc.node_interval == rc.node_interval / 2) 
							fprintf(outputfp[0], "%e\t%e\t%e\t%e\t%e\t%e\n", i * dh, laser.x, rc.mask[rc.maskIndex], rc.inputSantaFe[rc.inputIndex], rc.input_signal, laser.x);
						else fprintf(outputfp[0], "%e\t%e\t%e\t%e\t%e\n", i * dh, laser.x, rc.mask[rc.maskIndex], rc.inputSantaFe[rc.inputIndex], rc.input_signal);
						//outputfp_io = fopen("MZM in out.txt", "w");
//						fprintf(outputfp_io,"%e\t%e\t%e\t%e\t%e\n",i * dh, laser.x, laser.feedback, (laser.p.fb_str * laser.xdelay[laser.fbIndex[0]] + laser.p.phi) / M_PI, cos(laser.p.fb_str * laser.xdelay[laser.fbIndex[0]] + laser.p.phi)*cos(laser.p.fb_str * laser.xdelay[laser.fbIndex[0]] + laser.p.phi));
						fprintf(outputfp_io,"%e\t%e\t%e\t%e\t%e\n",i * dh, laser.x, laser.feedback, laser.p.fb_str * laser.xdelay[laser.fbIndex[0]], cos(laser.p.fb_str * laser.xdelay[laser.fbIndex[0]] + laser.p.phi)*cos(laser.p.fb_str * laser.xdelay[laser.fbIndex[0]] + laser.p.phi));
						//fclose(outputfp_io);
					}
					if(i == plotnum) break;
				}
			}
			//calc modulate up to here
			if(i % rc.node_interval == rc.node_interval / 2) {
				rc.nodeState[rc.inputIndex][rc.nodeIndex] = laser.x;
		...
		if(out == 1) {
			if(para_flag == 1) fclose(outputfp[0]), fclose(outputfp_io);
			if(para_flag == 2) {
				...
				fprintf(outputfp[0], "Index\tTarget\tTraining result\n");
				for(i = 0; i < rc.trainingNum; i++) fprintf(outputfp[0], "%d\t%e\t%e\n", i, rc.target[i + 1], rc.output[i]);
				fclose(outputfp[0]);
				
				EvaluateReservoirTest(&rc);
				outputfp[0] = fopen("rc_test_result.txt", "w");
				fprintf(outputfp[0], "Index\tTarget\tTest result\n");
				for(i = rc.trainingNum; i < rc.allNum; i++) fprintf(outputfp[0], "%d\t%e\t%e\n", i, rc.target[i + 1], rc.output[i]);
				fclose(outputfp[0]);
			}
			break;
		}
		if(out == 2) {
			OutputHistgram(&hist, "histgram.txt\0");
			QR(&rc);
			fprintf(outputfp[0], "%e\t%e\t%e\t%e\n", param, EvaluateReservoirTrain(&rc), EvaluateReservoirTest(&rc), hist.entropy);
...
//	rc->gamma = 1.0 / 255.0;
	...
	rc->inputSantaFe = (double*)malloc(sizeof(double) * 5000);
...
void ReadSantaFe(struct Reservoir *rc) {
	...
	while((ret = fscanf(inputfp, "%d", &data)) != EOF) {
		rc->inputSantaFe[count] = rc->target[count] = (double)data;
		count++;
	}
...
void QR(struct Reservoir *rc) {
	...
	for(i = 0; i < rc->node_num; i++) {
		...
		yx[i] = 0.0;
		for(j = 0; j < rc->trainingNum; j++) yx[i] += rc->target[j + 1] * rc->nodeState[j][i];
...
double EvaluateReservoirTrain(struct Reservoir *rc) {
	...
	for(i = 0, NMSE = sum_y = sqsum_y = 0.0; i < rc->trainingNum; i++) {
		sum_y += rc->target[i + 1];
		sqsum_y += rc->target[i + 1] * rc->target[i + 1];
		for(j = 0, one_y = 0.0; j < rc->node_num; j++) one_y += rc->weight[j] * rc->nodeState[i][j];
		NMSE += (rc->target[i + 1] - one_y) * (rc->target[i + 1] - one_y);
		rc->output[i] = one_y;
	}
...
void CalcLaserEOfeedback(struct LaserEOfeedback *laser, double input_signal) {
	...
	ini[0] = laser->x;
	ini[1] = laser->y;
	for(i = 0, laser->buffer = 0.0; i < laser->fbNum; i++) laser->buffer += laser->xdelay[laser->fbIndex[i]] * laser->p.weight_fb[i];
	
	// cosTheta = cos(laser->p.fb_str * buffer + laser->p.phi + input_signal);
	// feedback = laser->p.beta * cosTheta * cosTheta;
	
	/***** for beta modulation *****/
	cosTheta = cos(laser->p.fb_str * laser->buffer + laser->p.phi);
	beta_mod = 2.0 * cos(input_signal * M_PI / 4.0 - M_PI / 4.0) * cos(input_signal * M_PI / 4.0 - M_PI / 4.0);
	laser->feedback = laser->p.beta * cosTheta * cosTheta * beta_mod;
//	laser->feedback = laser->p.beta * cosTheta * cosTheta * 2.0;
	/***** for beta modulation *****/
	...
	for(i = 0; i < 4; i++) {
		...
		b[0][i] = interval_L * ( - (1.0 + laser->p.epsilon) * a[0] - a[1] + laser->feedback);
