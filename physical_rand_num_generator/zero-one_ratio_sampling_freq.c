...
int main()
{
  ...
  double threshold = 1.93; 

  double sample_freq = 0.1 * 1e9;
  double sample_min = 0.1 * 1e9;
  double sample_max = 10.1 * 1e9;
  int sample_num = 100;
  ...
  printf("Sampling frequency [GHz]\tOne ratio [arb.units]\n");

  //chg sampling interval
  for(j = 0; j < sample_num; j++){
	  sample_freq = sample_div * j + sample_min;
	  sInt = 1 / sample_freq;
	  num_zero = 0;
	  num_one = 0;
