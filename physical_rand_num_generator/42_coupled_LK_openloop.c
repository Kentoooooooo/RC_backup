// 4.2 一方向結合されたLang-Kobayashi方程式のカオス同期の時間波形（図4.14）
...
double kapInjRatio = 0.0; // injection strength ratio to drive feedback strength
...
int main()
{
  ...
  for(i = 0; i < n; i++){
    ...
    if(i % div == 0){
      ...
      printf("%e\t", a[0] * a[0] * 1e-20); // Drive Intensity
      printf("%e\t", a[3] * a[3] * 1e-20); // Response Intensity
      printf("%e\n", eDelayDri[delayDriIndex] * eDelayDri[delayDriIndex] * 1e-20); // Delayed Drive Intensity
    }
    rungeKutta(a, h, t);
  }
  return 0;
