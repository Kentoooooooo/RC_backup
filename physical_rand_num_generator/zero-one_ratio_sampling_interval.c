...
int main()
{
  ...
  double tMax      = 200.0e-9;   // plot time
  double threshold = 2.0; 

  double sInt      = 5.0e-12;   // sampling interval
  double sample_min = 5.0e-12;
  double sample_max = 755.0e-12;
  int sample_num = 750;
  double sample_div = (sample_max - sample_min) / sample_num;
    
  
  int div = 50; // plot interval
  ...
  printf("Sampling frequency [arb.units]\tOne ratio [arb.units]\t(One ratio - 0.5)\\u2\\n\n");

  //chg sampling interval
  for(j = 0; j < sample_num; j++){
	  sInt = sample_div * j + sample_min;
	  num_zero = 0;
	  num_one = 0;
  ...
  printf("%e\t%e\t%e\n", sInt * 1e+9, one_ratio, (one_ratio - 0.5) * (one_ratio - 0.5));
